...
function calcular_total_medalhas() {
    ...
    while (true) {
        ...
        if (pais.toLowerCase() == "sair")  //toLowerCase para aceitar qualquer escrita de "SAIR","Sair"...
          ...
          let ranking = Object.entries(medalhasPorPais).sort((a, b) => b[1] - a[1]); // Ordena em ordem decrescente

          let resultado = "Ranking de medalhas:\n"; // Adiciona nova linha após o título
          for (let i = 0; i < ranking.length; i++) {
              let pais = ranking[i][0]; 
              let total_medalhas = ranking[i][1]; // Usar o nome correto da variável
              resultado += pais + ": " + total_medalhas + " medalhas\n"; // Corrige o nome da variável e adiciona nova linha
          }
